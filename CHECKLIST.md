...
- [~] Portable presence/frequency penalties, logit bias, logprobs/top-logprobs, auto/default/flex/priority service tiers, and detached per-request extra headers are fieldwise merged. OpenAI Chat maps all supported fields; Responses maps logprobs and tiers; Gemini Developer API maps penalties, logprobs, and tier values; Vertex maps default/flex/priority tiers to capacity headers; Anthropic maps compatible tiers. Static and streamed OpenAI Chat/Gemini responses preserve logprobs and service metadata, while static and streamed Responses text preserves logprobs, raw annotations, and phases. OpenAI and Anthropic support detached extra request-body fields with typed-field conflict protection. Provider-specific typed overrides and remaining provider metadata remain.
...
- [~] Detached binary `FilePart` output, compatible history serialization, telemetry, static/streamed lifecycle events, and replacement deltas are complete. OpenAI Responses code interpreter and image generation plus Google image models emit generated images, with Google same-provider inline replay. Ordinary provider multimodal response content, citations, audio/video, and typed image-only output remain.
...
- [~] Focused instruction, output, provider, MCP, capability, history, usage, multimodal, native-tool, evaluation, and OpenTelemetry guides now keep advanced setup out of the README. Add guides for deferred execution, compaction, and model wrappers without turning the README into an exhaustive API dump.
...
5. Add the remaining provider-native tools, richer content parts, provider metadata, and OpenAI-compatible providers; dependency-driven native-tool factories, OpenAI Responses web search/file search/code execution/image generation/MCP servers, Gemini file search/code execution/web search/fetch/image generation, Anthropic memory/code execution/web search/fetch, Z.AI Chat Completions, OpenAI refusal handling, Google prompt-feedback blocks, system-prompt reinjection, tool return-schema advertisement, and the `pydantic-evals-go` task adapter are complete.
