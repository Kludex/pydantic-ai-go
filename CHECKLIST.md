...
- [~] OpenAI Responses supports automatic server-executed and keyword/custom client-executed `tool_search` in streaming and non-streaming requests, normalizes and replays nullable provider identities without guessing ambiguous pairs, executes same-response discovered calls, and sends other reveals through `additional_tools`. Anthropic renders deferred definitions, local search results as `tool_reference` blocks, and other reveals as `tool_addition` blocks; native Anthropic BM25/regex remains.
...
2. Add provider-managed search for Anthropic and OpenAI Responses; capability grouping, ordering constraints, and transparent leaf decorators are complete.
3. Add MCP now that raw/dynamic toolsets have run/step lifecycle and deferred calls have an explicit pause/resume boundary.
