...
Anthropic 4.5+ models advertise the hidden schemas with `defer_loading`, replay local search results as `tool_reference` blocks, and render other reveals as `tool_addition` blocks. OpenAI Responses does the same for non-streaming requests with client-executed `tool_search`, `tool_search_output`, and `additional_tools` items. This keeps the stable visible-tool prefix small without changing local execution safety. Streaming and non-streaming OpenAI Responses requests use the same native client-executed search protocol, including final streamed call IDs. Use each provider's `WithDeferredToolSupport(false)` option for compatible endpoints that do not implement its native wire protocol.
