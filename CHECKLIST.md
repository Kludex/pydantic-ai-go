...
- [~] OpenAI Responses: text/reasoning/function-call/compaction streaming plus authoritative terminal snapshots, response/item IDs, encrypted reasoning and compaction round trips, latest-compaction history trimming with standing-prompt provenance, stateful context management and explicit stateless `/responses/compact` with message/custom triggers, durable history replacement, combined usage accounting, pre-request `/responses/input_tokens` counting, refusal events/snapshots with inspectable content-filter responses, function namespaces, portable reasoning effort, logprob requests with static/streamed text metadata, raw streamed/static annotations, message phases with profile-gated same-provider replay and explicit compatibility overrides, service tiers, status, timestamps, configurable background create/poll/retrieve/cancel/detach continuation, and streaming/non-streaming server/client tool search with typed native history and identity-safe replay. Native output, multimodal content, and other builtin tools remain.
