...
- [~] `NativeToolCallPart` and `NativeToolReturnPart` are distinct non-executable response parts with upstream `builtin-tool-call`/`builtin-tool-return` serialization, provider identity, typed tool kinds, outcomes, timestamps, metadata, defensive cloning, and normalized streaming start/delta/return lifecycle. Provider-neutral `NativeTool`, `WebSearchTool`, `WithNativeTools`, `WithRunNativeTools`, `Agent.AddNativeTool`, and capability registration provide detached, duplicate-safe static scopes with required/optional compatibility behavior. Dependency-driven native-tool factories and remaining native tool types/provider rendering remain.
...
- [~] Provider-neutral web search is registered agent-wide, per-run, or by capabilities and rendered by OpenAI Responses with context size, location, allowed domains, live-web access, static/streamed call-return parts, and same-provider replay. Anthropic, Google, and other provider rendering; dynamic dependency-driven registration; web fetch; X search; code execution; file search; image generation; memory; MCP server; and advisor tools remain.
...
5. Add dependency-driven native-tool factories, Anthropic/Google web-search rendering, the remaining provider-native tools, richer content parts, provider metadata, and OpenAI-compatible providers; OpenAI Responses web search, Z.AI Chat Completions, OpenAI refusal handling, Google prompt-feedback blocks, system-prompt reinjection, tool return-schema advertisement, and the `pydantic-evals-go` task adapter are complete.
