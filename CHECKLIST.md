...
- [~] Provider-neutral web search is registered statically or through per-step dependency callbacks agent-wide, per-run, or by capabilities. OpenAI Responses renders context size, location, allowed domains, and live-web access with static/streamed call-return parts and same-provider replay. Gemini renders `googleSearch`, gates function/native combinations by generation, and normalizes static/streamed grounding queries and sources while retaining complete metadata. Anthropic selects model-family-aware tool versions, renders location/domain/use limits, preserves caller metadata, and normalizes static/streamed calls, results, errors, and replay. Other provider rendering; web fetch; X search; code execution; file search; image generation; memory; MCP server; and advisor tools remain.
...
5. Add the remaining provider-native tools, richer content parts, provider metadata, and OpenAI-compatible providers; dependency-driven native-tool factories, OpenAI Responses, Gemini, and Anthropic web search, Z.AI Chat Completions, OpenAI refusal handling, Google prompt-feedback blocks, system-prompt reinjection, tool return-schema advertisement, and the `pydantic-evals-go` task adapter are complete.
