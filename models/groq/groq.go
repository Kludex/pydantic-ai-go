...
func (model *Model) Request(
	...
	response, err := model.ModelWrapper.Request(ctx, messages, prepared)
	if failed, ok := model.toolUseFailedResponse(err); ok {
		return failed, nil
	}
	if err != nil {
		return nil, err
	}
	splitTaggedThinking(response)
	return response, nil
}
...
func (model *Model) StreamRequest(
	...
	stream, err := model.ModelWrapper.StreamRequest(ctx, messages, prepared)
	if failed, ok := model.toolUseFailedResponse(err); ok {
		return failedResponseEvents(failed), nil
	}
	if err != nil {
		return nil, err
	}
	return splitTaggedThinkingEvents(stream), nil
}
...
func (model *Model) toolUseFailedResponse(err error) (*ai.ModelResponse, bool) {
	...
	if json.Unmarshal([]byte(apiError.Body), &body) != nil {
		return nil, false
	}
	code, generation := body.Code, body.FailedGeneration
	if body.Error != nil {
		code, generation = body.Error.Code, body.Error.FailedGeneration
	}
	if code != "tool_use_failed" {
		return nil, false
	}
	response := &ai.ModelResponse{
		ModelName: model.name, Timestamp: time.Now().UTC(), ProviderName: model.ProviderName(),
		ProviderURL: model.ProviderURL(), FinishReason: ai.FinishReasonError, State: ai.ModelResponseStateComplete,
		ProviderDetails: map[string]any{"finish_reason": "error", "tool_use_failed": true},
	}
	if generation == "" {
	...
	if json.Unmarshal([]byte(generation), &failed) == nil && failed.Name != "" && json.Valid(failed.Arguments) {
		digest := sha256.Sum256([]byte(generation))
		response.Parts = append(response.Parts, ai.ToolCallPart{
			ToolName: failed.Name, Args: failed.Arguments, ToolCallID: fmt.Sprintf("groq-failed:%x", digest[:8]),
			ProviderName: model.ProviderName(),
		})
	} else {
		response.Parts = append(response.Parts, ai.TextPart{Content: generation, ProviderName: model.ProviderName()})
	}
	return response, true
}

func failedResponseEvents(response *ai.ModelResponse) iter.Seq2[ai.ModelStreamEvent, error] {
	return func(yield func(ai.ModelStreamEvent, error) bool) {
		for index, part := range response.Parts {
			partID := fmt.Sprintf("failed:%d", index)
			switch value := part.(type) {
			case ai.ToolCallPart:
				if !yield(ai.ToolCallStartEvent{
					PartID: partID, ToolName: value.ToolName, ToolCallID: value.ToolCallID,
					ProviderName: value.ProviderName,
				}, nil) || !yield(ai.ToolCallDeltaEvent{PartID: partID, ArgsDelta: string(value.Args)}, nil) {
					return
				}
			case ai.TextPart:
				if !yield(ai.TextDeltaEvent{
					PartID: partID, Delta: value.Content, ProviderName: value.ProviderName,
				}, nil) {
					return
				}
			}
		...
		yield(ai.FinishEvent{
			ModelName: response.ModelName, Timestamp: response.Timestamp, ProviderName: response.ProviderName,
			ProviderURL: response.ProviderURL, ProviderDetails: response.ProviderDetails,
