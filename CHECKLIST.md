...
- [~] OpenAI Responses: text/reasoning/function-call/compaction streaming plus authoritative terminal snapshots, response/item IDs, encrypted reasoning and compaction round trips, latest-compaction history trimming with standing-prompt provenance, stateful context management and explicit stateless `/responses/compact` with message/custom triggers, durable history replacement, combined usage accounting, pre-request `/responses/input_tokens` counting, refusal events/snapshots with inspectable content-filter responses, function namespaces, portable reasoning effort, logprob requests with static/streamed text metadata, raw streamed/static annotations, message phases, service tiers, status, timestamps, configurable background create/poll/retrieve/cancel/detach continuation, and streaming/non-streaming server/client tool search with typed native history and identity-safe replay. Phase replay with model-profile gating, native output, multimodal content, and other builtin tools remain.
...
- [~] Provider-neutral compaction boundaries, composable message history processors, token-aware request-only trimming through `TokenHistoryTrimmer`, stateful OpenAI Responses/Anthropic compaction, OpenAI stateless message/custom triggers, direct compaction requests, durable history replacement, model-wrapper unwrapping, and usage-limit accounting are complete. Dedicated compaction tracing and provider-neutral summarization helpers that preserve complete turns and attribute side-request usage remain.
...
- [~] The README now starts with concise, runnable agent-and-tool setup and includes verified structured-output, streaming, conversation-history, provider-selection, and fake-model examples. Focused instruction, output, provider, MCP, capability, history-trimming, usage-limit/token-counting, and OpenTelemetry guides cover stable source-qualified prompts, output functions and unions, OpenAI-compatible/Azure setup, shared-session lifecycle, direct protocol operations, privacy, format compatibility, and usage attribution. Dedicated multimodal examples remain.
- [~] Focused instruction, output, provider, MCP, capability, history, usage, and OpenTelemetry guides now keep advanced setup out of the README. Add guides for deferred execution, compaction, and model wrappers without turning the README into an exhaustive API dump.
...
2. Add provider-neutral summarization that preserves whole turns and attributes side-request usage; token-aware request-only trimming and provider-native compaction are complete.
