...
```go
func printPrice(response ai.ModelResponse) error {
...
Wrap request-only history transforms with `ai.HistoryProcessor`. Processors run in capability order and receive detached messages plus `RunInfo`. Their output changes the provider view without deleting durable history. This prevents retry, resume, or provider changes from accidentally inheriting a lossy slice.

Embed `ai.WrappedCapability` when you decorate one capability but only override selected hooks. Create it with `ai.WrapCapability(inner)`. Registration keeps the decorator immediately outside the inner capability, runs each setup once, adopts the inner ordering, and preserves every optional interface you did not override. Wrap individual leaves before passing them to `ai.CombineCapabilities(...)`.

Before hooks run in capability order. After and error hooks run in reverse order. This matches middleware nesting. An error hook may return a replacement response. Return `ai.Retryf(...)` from a before or after hook to consume the output retry budget and ask the model to respond again. A response rejected by an after hook remains in history. Set `ModelRequestContext.ReplaceHistory` when a hook's rewritten messages must become durable run history, and report side-request cost through `AdditionalUsage`; limits are checked before the primary request. `ModelRequestContext.Clone` detaches mutable request data and usage details when a hook needs to retain or inspect a snapshot.

Run middleware uses `BeforeRunHook`, `AfterRunHook`, `RunErrorHook`, and `RunWrapper`. A wrapper can call `next`, return `ai.CompletedRunOutcome(...)` without starting the loop, or recover an error. After hooks also transform recovered and short-circuit outcomes. `RunOutcome` keeps deferred pauses distinct from completed output. Cancellation is terminal even when a wrapper or error hook returns a recovery outcome. Stopping a stream unwinds wrappers for cleanup without invoking completion or error hooks.
...
Output hooks split structured parsing from final processing. Use `BeforeOutputValidationHook` to repair raw structured output, `AfterOutputValidationHook` to transform the parsed value, and the matching wrapper and error hook around schema validation and decoding. Plain text skips the validation lifecycle. Processing hooks receive the typed semantic value and wrap registered output validators. Processing runs for valid streaming partials and the final output; `OutputHookContext.Partial` distinguishes them. Return `ai.Retryf(...)` to use the output retry budget. A retry after stream commitment terminates the stream because committed output cannot start another model round.

Slice order is middleware order: the first capability is outermost. Implement `CapabilityOrderingProvider` when a reusable capability must control its position regardless of registration order. `CapabilityOutermost` and `CapabilityInnermost` create fixed tiers. `Wraps` and `WrappedBy` add relative edges with `CapabilityType[T]()` or `CapabilityInstance(...)`; `Requires` rejects an incomplete capability set. Ordering is stable, and cycles fail during setup.

Usage limits are implemented on this same surface internally. Pass `ai.WithRunCapabilities(...)` to scope setup contributions and middleware to one run. Agent capabilities remain outermost. Agent and run capability groups are sorted independently, but run requirements can be satisfied by agent capabilities. A run capability is set up once for that run and may contribute instructions, settings, and raw tools without modifying the shared agent.
...
Use `anthropic.NewCompaction(...)` for Anthropic's input-token trigger, optional summary instructions, and pause-after-compaction behavior. Both provider configurations are capabilities, so you can install them on an agent or one run. They preserve explicit context-management settings and reject the wrong model type. Compaction parts reset deferred-tool discoveries from the summarized window while later reveals remain active.
